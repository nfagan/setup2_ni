...
timing.gaze_triggered_delay = 0.5;
timing.gaze_delay_reward_m1 = 1;
timing.gaze_delay_reward_m2 = 1;
...
m1_to_monitor =49.0;%cm
m2_to_monitor = 51.5;%cm
...
%     end
  ...
  if ( enable_gaze_triggered_delay && rand() < trial_desc.prob_gaze_triggered_delay )
    ...
    if m2_fixated
      ...
      ['m2 second']
      [timing.gaze_delay_reward_m2* m2_fixated]
    end
...
%     draw_texture( signaler_win, cross_im, signaler_rect );
    ...
    draw_texture( signaler_win, cross_im, m2_centered_rect_screen(fix_cross_size_m2) );
    if ( draw_m2_eye_roi )
